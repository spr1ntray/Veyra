...
### BUG-029: passives.css — только 7 именованных цветов вместо 12

**Тип:** незначительный
**Статус:** открыт
**Дата обнаружения:** 2026-04-26
**Компонент:** src/css/passives.css — header комментарий, строки 3-12

**Описание:**
Спецификация требовала «палитра 12 цветов». Комментарий файла перечисляет 7 именованных токенов: `#0e0e18`, `#1a1a24`, `#3a3a4a`, `#e8b44a`, `#ff7a1a`, `#d8d2c0`, `#05050a`. Цвета stormcaller/tidecaster/geomancer акцентов (`#8b5cd6`, `#5c9fd6`, `#5cd66b`) фактически используются в CSS-правилах на строках 131-133, но не задокументированы в palette-комментарии. Ещё `#3a3a4a` и `#05050a` — итого 10 используемых уникальных значений. До 12 не дотягивает.

**Ожидаемое поведение:**
Документированная палитра покрывает все фактически используемые цвета.

**Рекомендация:**
Добавить в комментарий palette 5 оставшихся цветов: `#8b5cd6` (Storm Purple), `#5c9fd6` (Tide Blue), `#5cd66b` (Geo Green), и два дополнительных из активного CSS.

---

### BUG-030: getDefaultState() возвращает version: 4, но схема теперь v5

**Тип:** средний
**Статус:** открыт
**Дата обнаружения:** 2026-04-26
**Компонент:** src/js/state.js — getDefaultState()

**Описание:**
BUG-002 (version:2 вместо 3) был актуален до рефакторинга. После введения миграции v4→v5 в этой сессии аналогичная проблема возможна: если `getDefaultState()` возвращает версию ниже 5, новый игрок при каждой загрузке будет проходить ветку v5-миграции (которая ищет старые U1..P-K2 IDs, не находит их и делает refund=0 — безвредно, но избыточно).

**Шаги воспроизведения:**
```js
JSON.parse(localStorage.getItem('veyra_player')).version
// → 4 (если getDefaultState ещё не обновлён на 5)
```

**Рекомендация:**
Проверить что `getDefaultState()` возвращает `version: 5`. Если нет — обновить.
...
*Последнее обновление: 2026-04-26 (PM-сессия верификация: пп.1-7 — все PASS, добавлены BUG-029, BUG-030)*
